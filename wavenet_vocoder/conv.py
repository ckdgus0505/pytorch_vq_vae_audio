...
class Conv1d(nn.Conv1d):
    ...
    def incremental_forward(self, input_data):
        ...
        bsz = input_data.size(0)  # input: bsz x len x dim
        if kw > 1:
            input_data = input_data.data
            if self.input_buffer is None:
                #.new creates a tensor of the same data type on the same device as the original tensor
        ...
        #print("input_data.shape", input_data.shape)
        output = F.linear(input_data.view(bsz, -1), weight, self.bias)
