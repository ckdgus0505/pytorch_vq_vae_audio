...
class WaveNet(nn.Module):
    ...
    Args:
        ...
        decoding_channels (list): list of channel numbers used for local conditioning upsampling.
    ...
    def __init__(self, device,out_channels=256, layers=20, stacks=2,
        ...
        if gin_channels > 0 and use_speaker_embedding:
            ...
            assert gin_channels == n_speakers
...
#        print("x.shape", x.shape)
    ...
    def incremental_forward(self, initial_input=None, c=None, g=None,
        ...
        """
        self.clear_buffer()
        assert len(initial_input.shape) == 3
        B = 1
        ...
        if test_inputs is not None:
            ...
            #print("test_inputs.reshaped", test_inputs.shape)
        ...
        #print("global conditioning shape", g_btc.shape)
        # Local conditioning
        if c is not None and self.upsample_conv is not None:
            for f in self.upsample_conv:
                c = f(c)
            assert c.size(-1) == T
            #print("local conditioning shape", c.shape)
        ...
        if initial_input is None:
            ...
            if initial_input.size(1) == self.out_channels:
                #print("initial_input.shape0", initial_input.shape)
                initial_input = initial_input.transpose(1, 2).contiguous()
        #print("initial_input.shape", initial_input.shape)
